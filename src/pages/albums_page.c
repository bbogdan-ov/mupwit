...
static void entry_draw(size_t idx, AlbumEntry *entry, State *state) {
	Rect rect = {
	...
	draw_box(state, BOX_3D, artwork_rect, THEME_BLACK);
	offset.y += artwork_rect.height + GAP;

	// Draw title and artist
	Text text = {
		.text = entry->artist ? entry->artist : UNKNOWN,
	...
	while (true) {
		...
		if (strcmp(pair->name, "Album") == 0 && strlen(pair->value) > 0) {
			AlbumEntry entry = {
				...
				.artist = cur_artist ? strdup(cur_artist) : NULL,
			};
