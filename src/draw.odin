...
Context :: struct {
	...
	screen:      Rect,
	container:   Rect, // Current container rect. Elements should adapt to this container.
...
begin_container :: proc(ctx: ^Context, rect: Rect) -> (prev: Rect) {
	prev = ctx.container
	ctx.container = rect
	return prev
}
_container_end :: proc(ctx: ^Context, _, prev: Rect) {
	ctx.container = prev
}
