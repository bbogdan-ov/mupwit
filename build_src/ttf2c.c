// Build "script" that loads fonts and converts them into a ./build/fonts.c file
...
void generate_font(FILE *file, const char *name, const char *path, int base_size) {
	...
	fprintf(file, "static Rectangle %s_RECTS[] = {\n", name);
	...
	fprintf(file, "static GlyphInfo %s_GLYPHS[] = {\n", name);
	for (int i = 0; i < CODEPOINT_COUNT; i++) {
		fprintf(
			file,
			"{%d,%d,%d,%d,{0}},\n",
	...
	fprintf(file, "static unsigned char %s_IMAGE_DATA[] = {\n", name);
...
int main() {
	FILE *output_file = fopen("./build/fonts.c", "w");
	fprintf(
		output_file,
		"// Generated by 'build_src/ttf2h.c'\n"
		"// Every font image data is 2 color and 1 component (R8)\n"
		"#pragma once\n"
		"#include <raylib.h>\n"
		"\n"
	);
